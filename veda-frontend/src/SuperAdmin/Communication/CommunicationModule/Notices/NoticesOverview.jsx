...
export default function NoticesOverview() {
    ...
    const loadNotices = async () => {
      ...
      try {
        ...
        const mapped = data.map((n) => ({
          ...
          attachment:
            n.attachment?.filename || null,

          isRead:
            n.viewedBy?.some(
              (v) =>
                v?.userId === currentUser.id ||
                v?.clerkId === currentUser.id
            ) || false,
  ...
  const filteredNotices = notices.filter((notice) => {
    ...
    const matchesSearch =
      ...
      notice.sender.toLowerCase().includes(search);

    const matchesRole =
      filterRole === "all" ||
      notice.roles.includes(filterRole);
  ...
  const sentCount = notices.filter(
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-gray-500">
                    ...
                    <div className="flex items-center gap-1">
                      <FiCalendar />
                      <span>
                        Sent: {formatDate(notice.sentDate)}
                      </span>
                    </div>
